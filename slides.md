...
- [TL;DR List of Recommendations](#tldr-list-of-recommendations)
- [Outro](#outro)
- [References](#references)

<a
  class="github-fork-ribbon" href="https://github.com/noahp/gnu-make-tips"
...
1. *recursive expansion*: `variable = expression`  : assign expression on the
   right hand side verbatim to the variable- this behaves like a macro in C/C++,
   where the expression is evaluated when the variable is used:
   ```makefile
   FOO = 1
   BAR = $(FOO)
   FOO = 2
   ...
   $(info BAR=$(BAR))
   ```
2. *simple expansion*: `variable := expression` : assign the *result* of an
...
# overriding variable, yields the overriding value
...
all.zip: foo.txt test.txt
	# run the gzip command with all the prerequisites "$^",
...
```makefile
# Say our project builds a program and a library 'foo' and 'foo.a';
# if we want to build both by default we might make an 'all' rule
# that builds both
.PHONY: all
...
`.PHONY` where it's defined. See the following example.

```makefile
# the 'all' rule that builds and tests. Note that it's listed first
# to make it the default rule
.PHONY: all
all: build test
...
foo.a: foo-lib.c
	...
	# use ar to create a static library containing our object file.
	# use the '$@' variable here to specify the rule target 'foo.a'
	$(AR) rcs $@ foo-lib.o
```
...
```makefile
# a phony rule that builds our project; just contains a prerequisite
# of the library + program
.PHONY: build
build: foo foo.a

# a phony rule that runs our test harness. has the 'build' target as
# a prerequisite! Make will make sure (pardon the pun) the build
# rule executes first
.PHONY: test
test: build
	./run-tests.sh
...
Used when a single recipe generates multiple targets. For example, if you have a
script that generates multiple header files, you might use a Grouped Target.

The syntax is the same as normal target declaration, but you use the `&:`
separator instead of `:` (`&` indicating "all"):
...
# Generate multiple headers using generate-headers.py
$(AUTOGENERATED_HEADERS) &:
...
https://www.gnu.org/software/make/manual/html_node/Multiple-Targets.html#index-grouped-targets
...
```makefile
# this will compile 'test.c' with the default $(CC), $(CFLAGS), into
# the program 'test'. it will handle prerequisite tracking on test.c
test: test.o
```
...
# '$^' automatic variable, specifying ALL the prerequisites (all
# the OBJ_FILES) should be part of the link command
program: $(OBJ_FILES)
    $(CC) -o $@ $^
```
...
The basic form for .d prerequisites might be:
...
These prerequisites' recipes will run in order; but if they are newer than the
target, they will not trigger a target re-build.
...
# output the .o to the build directory, which we add as an
# order-only prerequisite- anything right of the | pipe is
# considered order-only
...
clean:
	...
	@# prefixing the shell comment lines '#' here prevents them from
...
# you can combine function calls; here we strip the suffix off of
# $(FILES) with the $(basename) function, then add the .o suffix
O_FILES=$(addsuffix .o,$(basename $(FILES)))

# note that the GNU Make Manual suggests an alternate form for
...
Prefixing with a `-`, `-include sources.mk`, will prevent an error if the
included file can't be found (eg if you generate it as part of a build).
...
```makefile
# This makefile should be invoked from the temporary build
# directory, eg:
...
# now inform Make we should look for prerequisites from the root
# directory as well as the cwd
VPATH += $(MAKEFILE_DIR)
...
```makefile
# now we can continue as if Make was running from the root
...
# pattern rule; since we added ROOT_DIR to VPATH, Make can find
# prerequisites like `src/test.c` when running from the build
# directory!
%.o: %.c
...
tools-unpacked.dummy: $(TOOLS_ARCHIVE)
	# running this command results in a directory.. but how do we
	# know it completed, without a file to track?
	tar xzvf $^
...
# The build folder, for all generated output. This should normally
# be included in a .gitignore rule
BUILD_FOLDER := build

# Default all rule will build the 'example' target, which here is an
# executable
.PHONY:
...
---
name: tldr-list-of-recommendations

## TL;DR List of Recommendations
